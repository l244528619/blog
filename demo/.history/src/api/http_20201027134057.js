...
// request.defaults.headers.common['headerStore'] = ''
...
request.interceptors.request.use((config) => {
   ...
   console.log(config.headers.headerStore)
   config.headers.Authorization = token;
//    if (!config.headers.hasOwnProperty('headerStore') && token) {
...
request.interceptors.response.use(
    ...
    response => {
        ...
        if (response.status == 200) {
            ...
            // if (response.data.flag) {					//如果服务器对用户验证通过返回“S”成功标签就将用户需要的数据传递给请求的地方
