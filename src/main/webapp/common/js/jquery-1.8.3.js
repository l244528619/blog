	...
	jQuery.extend({
		...
		each: function (obj, callback, args) {
			...
			if (args) {
				...
				if (isObj) {
					for (name in obj) {
						if (callback.call(obj[name], name, obj[name])
							=== false) {
							break;
						}
					}
		...
		makeArray: function (arr, results) {
			...
			if (arr != null) {
				...
				if (arr.length == null || type === "string" || type
					=== "function" || type === "regexp" || jQuery.isWindow(
						arr)) {
		...
		access: function (elems, fn, key, value, chainable, emptyGet, pass) {
			...
			if (key && typeof key === "object") {
				...
				if (fn) {
					for (; i < length; i++) {
						fn(elems[i], key,
							exec ? value.call(elems[i], i, fn(elems[i], key))
								: value, pass);
					}
				}
 ...
 */
	...
	jQuery.extend({
		...
		// Unique for each copy of jQuery on the pagetype
...
// checks a cache object for emptiness
	...
	jQuery.extend({
		...
		attr: function (elem, name, value, pass) {
			...
			} else if (hooks && "get" in hooks && notxml && (ret = hooks.get(
				elem, name)) !== null) {
				return ret;
		...
		prop: function (elem, name, value) {
			...
			if (value !== undefined) {
				if (hooks && "set" in hooks && (ret = hooks.set(elem, value,
					name)) !== undefined) {
					return ret;
 ...
 */
	jQuery.event = {
		...
		add: function (elem, types, handler, data, selector) {
			...
			if (!eventHandle) {
				elemData.handle = eventHandle = function (e) {
					...
					// when an event is called after a pagetype has unloaded
			...
			if (!onlyHandlers && !event.isDefaultPrevented()) {
				...
				if ((!special._default || special._default.apply(
					...
					if (ontype && elem[type] && ((type !== "focus" && type
						!== "blur") || event.target.offsetWidth !== 0)
		...
		dispatch: function (event) {
			...
			for (i = 0;
				i < handlerQueue.length && !event.isPropagationStopped(); i++) {
					...
					function (id, context, xml) {
						if (typeof context.getElementById !== strundefined
							...
							return m ?
								...
								!== strundefined && m.getAttributeNode(
									"id").value === id ?
									[m] :
									undefined :
								[];
				...
				if ((matcher = Expr.relative[tokens[i].type])) {
					...
					if (matcher[expando]) {
						...
						return setMatcher(
							...
							i > 1 && tokens.slice(0, i - 1).join("").replace(
							rtrim, "$1"),
							matcher,
				...
				if (match.length === 1) {
					...
					if (tokens.length > 2 && (token = tokens[0]).type === "ID"
						...
						context = Expr.find["ID"](
							token.matches[0].replace(rbackslash, ""), context,
							xml)[0];
						if (!context) {
							return results;
						}
...
// from a document (should be improved, where feasible).
	...
	jQuery.each({
		...
		jQuery.fn[name] = function (until, selector) {
			...
			ret = this.length > 1 && !guaranteedUnique[name] ? jQuery.unique(
				ret) : ret;
	...
	jQuery.extend({
		clone: function (elem, dataAndEvents, deepDataAndEvents) {
			...
			if ((!jQuery.support.noCloneEvent || !jQuery.support.noCloneChecked)
				...
				// elements on the pagetype has a name or id of "length"
			...
			if (fragment) {
				...
				handleScript = function (elem) {
					...
					if (!elem.type || rscriptType.test(elem.type)) {
						...
						return scripts ?
							scripts.push(
								elem.parentNode ? elem.parentNode.removeChild(
									elem) : elem) :
				...
				for (i = 0; (elem = ret[i]) != null; i++) {
					...
					if (!(jQuery.nodeName(elem, "script") && handleScript(
						elem))) {
	...
	function augmentWidthOrHeight(elem, name, extra, isBorderBox) {
		...
		for (; i < 4; i += 2) {
			...
			if (isBorderBox) {
				...
				if (extra !== "margin") {
					val -= parseFloat(
						curCSS(elem, "border" + cssExpand[i] + "Width")) || 0;
				}
			} else {
...
// Animations created synchronously will run synchronously
	...
	function defaultPrefilter(elem, props, opts) {
		...
		if (length) {
			dataShow = jQuery._data(elem, "fxshow") || jQuery._data(elem,
				"fxshow", {});
...
// in a pagetype that all might call define(). The loader will indicate
